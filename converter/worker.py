...
class ConversionWorker(QThread):
    ...
    def run(self):
        ...
        expanded: list[tuple] = []  # (src, dst_dir, fmt, quality, pil_image_or_none, stem)
        for job in self._jobs:
            ...
            if ext == ".pdf" and job.pdf_pages is not None:
                for idx in job.pdf_pages:
                    ...
                    expanded.append((job.src_path, job.dst_dir, job.output_format, job.quality, idx, stem))
            else:
                expanded.append((job.src_path, job.dst_dir, job.output_format, job.quality, None, None))

        total = len(expanded)
        for i, (src, dst_dir, fmt, quality, pdf_page_idx, stem) in enumerate(expanded):
            if self._cancelled:
                break
            try:
                pil_image = render_pdf_page(src, pdf_page_idx) if pdf_page_idx is not None else None
                dst = convert_image(src, dst_dir, fmt, quality, pil_image=pil_image, stem_override=stem)
